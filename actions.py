...
def retrieveDataFromNeo4j(cyber):
    ...
    return '[执行Neo4j查询操作, 在actions.py中进行更新] - ' +cyber
...
class ViewCaseDefendants(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message("[查询案件被告人, 在actions.py中进行更新] - " + case)
...
class ViewCaseDefendantsNum(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message("[查询涉案人员数目, 在actions.py中进行更新] - " + case)
...
class ViewDefendantData(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message("[查询涉案人员信息, 在actions.py中进行更新] - " + defendant)
...
class ViewCaseDetail(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        # dispatcher.utter_message(response)
        dispatcher.utter_message("[查询案件详情, 在actions.py中进行更新] - " + case)
        return [SlotSet('case',case)]
